            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center justify-between p-3 bg-primary-50 rounded-lg">
                    ...
                    <div class="text-right">
                        <p class="text-lg font-bold text-primary-700">{{ number_format($evaluasiTerbaru->c1_produktivitas ?? 0, 1) }}</p>
                        <div class="w-16 bg-gray-200 rounded-full h-1.5">
                            <div class="bg-primary-500 h-1.5 rounded-full" style="width: {{ min(100, ($evaluasiTerbaru->c1_produktivitas ?? 0) / 100 * 100) }}%"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between p-3 bg-success-50 rounded-lg">
                    ...
                    <div class="text-right">
                        <p class="text-lg font-bold text-success-700">{{ number_format($evaluasiTerbaru->c2_tanggung_jawab ?? 0, 1) }}</p>
                        <div class="w-16 bg-gray-200 rounded-full h-1.5">
                            <div class="bg-success-500 h-1.5 rounded-full" style="width: {{ min(100, ($evaluasiTerbaru->c2_tanggung_jawab ?? 0) / 100 * 100) }}%"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between p-3 bg-secondary-50 rounded-lg">
                    ...
                    <div class="text-right">
                        <p class="text-lg font-bold text-secondary-700">{{ number_format($evaluasiTerbaru->c3_kehadiran ?? 0, 1) }}</p>
                        <div class="w-16 bg-gray-200 rounded-full h-1.5">
                            <div class="bg-secondary-500 h-1.5 rounded-full" style="width: {{ min(100, ($evaluasiTerbaru->c3_kehadiran ?? 0) / 100 * 100) }}%"></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between p-3 bg-warning-50 rounded-lg">
                    ...
                    <div class="text-right">
                        <p class="text-lg font-bold text-warning-700">{{ number_format($evaluasiTerbaru->c4_pelanggaran ?? 0, 0) }}</p>
                ...
                <div class="flex items-center justify-between p-3 bg-danger-50 rounded-lg">
                    ...
                    <div class="text-right">
                        <p class="text-lg font-bold text-danger-700">{{ number_format($evaluasiTerbaru->c5_terlambat ?? 0, 0) }}</p>
                        <p class="text-xs text-gray-600">kali</p>
                    </div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('pegawai.history.index') }}" class="text-primary-600 hover:text-primary-700 text-sm font-medium">
                    Lihat Semua
                </a>
            </div>
                ...
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div class="flex items-center space-x-4">
                        <div class="flex-shrink-0">
                            ...
                            @endphp
                            <div class="h-10 w-10 bg-{{ $rankingColor }}-100 rounded-full flex items-center justify-center">
                                <span class="text-{{ $rankingColor }}-600 font-bold text-sm">#{{ $evaluasi->ranking }}</span>
                            </div>
                        </div>
                        <div>
                    ...
                    <div class="text-right">
                        <p class="text-lg font-bold text-gray-900">{{ number_format($evaluasi->total_skor, 2) }}</p>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const trendData = @json($trendData ?? [
        { period: 'Jan 2025', score: 142.5 },
        { period: 'Feb 2025', score: 138.2 },
        { period: 'Mar 2025', score: 145.1 },
        { period: 'Apr 2025', score: 148.7 },
        { period: 'Mei 2025', score: 151.2 },
        { period: 'Jun 2025', score: ($skorTerbaru ?? 155.0) }
    ]);
...
function downloadReport() {
    ...
    window.location.href = '{{ route("pegawai.evaluasi.download") }}';
}
</script>
@endpush
@endsection